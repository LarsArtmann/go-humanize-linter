...
| `pattern_generated.go`                     | `IsGeneratedFile` — gogenfilter-driven detector covering sqlc (via `.sql.go` suffix, content markers, or declared sqlc output dirs — NOT bare default names like `models.go`, which gogenfilter deliberately ignores as unreliable), templ, protobuf, deepcopy-gen, wire, moq, mockgen, mockery, easyjson, counterfeiter, plus the standard `// Code generated by` fallback. Includes a legacy-suffix fallback (`_gen.go`, `.gen.go`, `_templ.go`) for files gogenfilter does not know about. |
