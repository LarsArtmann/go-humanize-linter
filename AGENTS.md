...
**All 4 sibling deps are PUBLIC (verified 2026-09-08 via anonymous `go get` through proxy.golang.org).** `GOPRIVATE`/`GONOSUMDB` are legacy from when the deps were private — harmless but unnecessary; public proxy + sum.golang.org resolution works without them. The CI deploy-key infrastructure (see Gotchas) is likewise legacy and still functional but no longer required.
...
- **CI auth = per-repo read-only deploy keys + system ssh config (LEGACY, still active)** - Set up when the 4 deps (`go-linter-sdk`, `go-finding`, `gogenfilter`, `go-error-family`) were private; they are public now (verified 2026-09-08), so anonymous proxy fetch works and this machinery is unnecessary — removal is pending cleanup. Mechanics if it ever matters again: each repo had its OWN read-only deploy key (GitHub forbids reusing one deploy key across repos; a valid-but-other-repo key gets `ERROR: Repository not found` and ssh stops offering further keys, so sequential agent keys CANNOT work). Secrets `DEPLOY_KEY_GHL/_SDK/_FINDING/_GOGF/_ERRFAM` hold the private keys; the "Setup per-repo deploy keys" workflow step writes them to `~/.ssh`, registers `Host gh_*` aliases in `/etc/ssh/ssh_config` (system-wide, NOT `~/.ssh/config`: the go command sanitizes the env of the git subprocesses it spawns for module downloads, so ssh never reads the user config), and maps each module URL via `git config --global url.<alias>:...insteadOf`. Alias names use UNDERSCORES in both places.
